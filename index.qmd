...
x = np.logspace(start = 0, stop = 4, num = 10**5)
y1 = 1./np.sqrt(x)
y2 = [Q_rsqrt(i) for i in x]
err = abs(y2-y1)/y1
ax.plot(x, err)

plt.title("L'erreur relative entre l'inverse de la racine carrée et Q_rsqrt")
plt.xlabel("x")
ax.set_xscale('log')
plt.show()
```
...
![La méthode de Newton animée](https://upload.wikimedia.org/wikipedia/commons/e/e0/NewtonIteration_Ani.gif)
...
ax.plot(x, y)
plt.xlabel("Nombre à virgule flottante", fontsize=15)
plt.ylabel("Représentation comme un entier", fontsize=15)
plt.show()
```
...
ax.plot(x, y1, color = "blue", label = r"$log_2(1+x)$")
...
ax.plot(x, y1, color = "blue", label = r"$log_2(1+x)$")
ax.plot(x, x, color = "red", label = "x")
ax.plot(x, y2, color = "black", label = r"$x+\frac{1}{2}-\frac{1+\ln(\ln(2))}{2\ln(2)}$")
plt.xlabel("x",fontsize=15)
plt.legend(fontsize=15)
plt.show()
```
...
- La valeur choisie est $\sigma\approx0,04504656791687012$.
...
$$
E_x\cdot L \approx L\cdot \log_2 (N_x) + L\cdot B - M_x - L\cdot \sigma 
$$

. . .

$$
E_x\cdot L \approx L\cdot \log_2 (N_x) - M_x +L(B-\sigma) 
$$
...
\draw ([shift=({0,0cm})]f-0.east) node[right] {\Large $= 3,141592654$};
...
\draw ([shift=({0,0cm})]f-0.east) node[right] {\Large $= 1078530011$};
...
& \node (g-29) [exposant] {\Large 1}; 
& \node (g-28) [exposant] {\Large 0}; 
...
\draw ([shift=({0,0cm})]g-0.east) node[right] {\Large $= 539265005$};
...
& \node (f-2) [mantisse] {\Large 0};
& \node (f-1) [mantisse] {\Large 1};
& \node (f-0) [mantisse] {\Large 0};  
\\
};

% \draw ([shift=({0,0.5cm})]f-16.north) node[above] {\Large \emph{Magic number} $0x5F3759DF$};

\draw ([shift=({0,0cm})]f-0.east) node[right] {\Large $= 1058198002$};
...
\draw ([shift=({0,0cm})]f-0.east) node[right] {\Large $= 0,573516$};
...
\draw ([shift=({0,0cm})]f-0.east) node[right] {\Large $= 0,56395704$};
...
# Devrait-on utiliser cette fonction? {background-color="black"}
